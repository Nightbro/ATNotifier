...
internal sealed class SetupWizard : Form
{
    ...
    private readonly TextBox dateField = new() { Width = 250, Text = "Date", PlaceholderText = "Date" };
    ...
    private SetupWizard(string configurationPath)
    {
        ...
        Add(panel, "SharePoint site URL", site); Add(panel, "List title", list); Add(panel, "Date field (internal name)", dateField); Add(panel, "Check", day); Add(panel, "Notify on failure", notify);
    ...
    private void Save()
    {
        if (!Uri.TryCreate(site.Text.Trim(), UriKind.Absolute, out _)) { Error("Enter a valid SharePoint site URL."); return; } if (string.IsNullOrWhiteSpace(list.Text)) { Error("Enter the SharePoint list title."); return; } if (string.IsNullOrWhiteSpace(dateField.Text)) { Error("Enter the SharePoint date field's internal name."); return; }
        ...
        var config = new NotifierConfiguration { Notifications = channels.HasFlag(NotificationChannel.Email) ? new NotificationSettings { Email = new SmtpSettings { Host = smtp.Text.Trim(), From = from.Text.Trim(), To = [to.Text.Trim()] } } : new NotificationSettings(), Checks = [new CheckConfiguration { Name = list.Text.Trim(), Notify = channels, SharePoint = new SharePointListCheckSettings { SiteUrl = site.Text.Trim(), ListTitle = list.Text.Trim(), DateField = dateField.Text.Trim(), Day = (DayRange)day.SelectedItem!, Authentication = other.Checked ? new SharePointAuthentication { Mode = AuthenticationMode.ConfiguredCredentials, Username = user.Text.Trim(), PasswordSecretName = secret } : new SharePointAuthentication() } }] };
